...
+ (instancetype)share {
    static MWDeviceUtil *device = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        device = [[MWDeviceUtil alloc] init];
    });
    return device;
}

/*
 deviceId idfa/androidid
 fingerPrint 指纹
 os 操作系统
 osVersion 操作系统版本
 model 型号
...
- (NSDictionary *)getAllDevice
{
    ...
    NSDictionary *device = @{
                             MW_POST_KEY_DEVICE_DEVICE_ID:Value_class(self.deviceId), // deviceId
                             MW_POST_KEY_DEVICE_FP:Value_class(self.fingerPrint),     // fingerPrint
                             MW_POST_KEY_DEVICE_OS:@(self.os),                        // os
                             MW_POST_KEY_DEVICE_OSVS:Value_class(self.osVersion),     // osVersion
                             MW_POST_KEY_DEVICE_MODEL:Value_class(self.model),        // model
                             MW_POST_KEY_DEVICE_CARRIER:@(self.carrier),              // carrier
                             MW_POST_KEY_DEVICE_FIRST_TIME:Value_class(self.appFirstStartTime),    // 该设备首次使用sdk时间
                             MW_POST_KEY_DEVICE_FIRST_LAUNCH:@(self.isNewDevice),     // 如果为新设备，则为0，否则为1
                             };
    return [MWDictionaryUtils reviewDic:device];
}
...
- (int)isNewDevice {
    BOOL isUpdateDevice = [[MWCommonService sharedInstance] isUpdateDevice];
    return isUpdateDevice ? 0: 1;
}
